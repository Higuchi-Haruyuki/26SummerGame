...
Item UIManager::GetSelectedHalfItemType() const
{
	const auto& safeItemSlot = m_halfReferenceItemSlot.lock();
	if (!safeItemSlot) return Item::kNone;

	if (m_halfReferenceIndex == -1) return Item::kNone;

	auto item = safeItemSlot->GetItem(m_halfReferenceIndex);
	if (!item) return Item::kNone;
